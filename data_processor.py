...
class F1FeatureEngineer:
    ...
    def create_driver_features(self, race_results: pd.DataFrame, qualifying_results: pd.DataFrame) -> pd.DataFrame:
        ...
        for driver in drivers:
            ...
            for i, race in driver_races.iterrows():
                ...
                features = {
                    ...
                    'driver_career_avg_pos': historical_races['position'].apply(
                        lambda x: float(x) if isinstance(x, (int, float)) else 20
                    ).mean() if len(historical_races) > 0 else 20,

                    # Recent stats
                    'driver_recent_avg_pos': recent_races['position'].apply(
                        lambda x: float(x) if isinstance(x, (int, float)) else 20
                    ).mean() if len(recent_races) > 0 else 20,
                    'driver_recent_points': recent_races['points'].sum(),
                    'driver_recent_wins': len(recent_races[recent_races['position'] == 1]),
                    'driver_recent_podiums': len(recent_races[recent_races['position'].isin(['1','2','3'])]),

                    # Qualifying stats
                    'driver_avg_qual_pos': recent_quals['position'].mean() if len(recent_quals) > 0 else 20,
                    'driver_qual_improvement': (
                        recent_quals['position'].mean() - driver_quals['position'].mean()
                    ) if len(driver_quals) > 0 and len(recent_quals) > 0 else 0,

                    # Track-specific performance
                    'circuit_id': race_results[race_results['round'] == race['round']]['circuit_id'].iloc[0] if 'circuit_id' in race_results.columns else 'unknown',

                    # Targets
                    'race_position': float(race['position']) if isinstance(race['position'], (int, float)) else 20,
    ...
    def create_constructor_features(self, race_results: pd.DataFrame) -> pd.DataFrame:
        ...
        for constructor in constructors:
            constructor_races = race_results[race_results['constructor_id'] == constructor].copy()
            constructor_races = constructor_races.sort_values(['year','round'])
            ...
            for year in constructor_races['year'].unique():
                for round_num in constructor_races[constructor_races['year'] == year]['round'].unique():
                    ...
                    features = {
                        ...
                        'constructor_career_podiums': len(historical_races[historical_races['position'].isin(['1','2','3'])]),
                        'constructor_career_points': historical_races['points'].sum(),
                        'constructor_avg_pos': historical_races['position'].apply(
                            lambda x: float(x) if isinstance(x, (int, float)) else 20
                        ).mean() if len(historical_races) > 0 else 20,

                        # Recent performance
                        'constructor_recent_avg_pos': historical_races['position'].apply(
                            lambda x: float(x) if isinstance(x, (int, float)) else 20
                        ).mean() if len(recent_races) > 0 else 20,
                        'constructor_recent_points': recent_races['points'].sum(),
                        'constructor_recent_wins': len(recent_races[recent_races['position'] == '1']),
                        'constructor_recent_podiums': len(recent_races[recent_races['position'].isin(['1','2','3'])]),
                    }
    ...
    def create_track_features(self, race_results: pd.DataFrame, race_schedule: pd.DataFrame) -> pd.DataFrame:
        ...
        track_features = []
        circuits = race_results_with_circuit['circuit_id'].unique()

        for circuit in circuits:
            if pd.isna(circuit):
                continue

            circuit_races = race_results_with_circuit[
                race_results_with_circuit['circuit_id'] == circuit
            ].copy()
            circuit_races = circuit_races.sort_values(['year','round'])

            for year in circuit_races['year'].unique():
                round_num = circuit_races[circuit_races['year'] == year]

                # Historical races at this specific circuit
                historical_races = circuit_races[circuit_races['year'] < year]

                features = {
                    'year': year,
                    'round': round_num,
                    'circuit_id': circuit,
                    'circuit_name': circuit_races['circuit_name'].iloc[0],
                    'country': circuit_races['country'].iloc[0],

                    # Track characteristics
                    'track_total_races': len(historical_races),
                    'track_avg_winner_pos': historical_races[historical_races['position'] == '1']['grid'].mean() if len(historical_races) > 0 else 1,
                    'track_overtaking_difficulty': (
                        historical_races.groupby(['year','round'])['grid'].apply(
                            lambda x: (x - historical_races.loc[x.index, 'position'].apply(
                                lambda y: float(y) if str(y).isdigit() else 20
                            )).abs().mean()
                        ).mean()
                    ) if len(historical_races) > 0 else 0,

                    # Track-specific driver performance
                    'track_repeat_winners': len(historical_races[historical_races['position'] == '1']['driver_id'].value_counts()) if len(historical_races) > 0 else 0
                }
        ...
        for (year, round_num), race_group in race_results.groupby(['year','round']):
            ...
            for _, driver_result in race_group.iterrows():
                ...
                features = {
                    ...
                    'grid_position_norm': driver_result['grid'] / 20,
                    ...
                    'qual_position': driver_qual['position'].iloc[0] if len(driver_qual) > 0 else 20,
                    'qual_to_grid_diff': (
                        driver_qual['position'].iloc[0] - driver_result['grid']
                    ) if len(driver_qual) > 0 else 0,
                    ...
                    # Targets
                    'race_position': float(driver_result['position']) if isinstance(driver_result['position'], (int, float)) else 20,
                    'race_points': driver_result['points'],
                    'position_change': (
                        driver_result['grid'] - float(driver_result['position'])
                    ) if isinstance(driver_result['position'], (int, float)) else -20,
    ...
    def prepare_datasets(self, features_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        target_cols = [
            'race_condition', 'race_points', 'race_win', 'race_podium', 'points_finish'
        ]
        ...
        train_years = features_df['year'].unique()
        train_years = train_years[train_years < max(train_years)]

        train_mask = features_df['year'].isin(train_years)

        X_train = X[train_mask] # training set
        y_train = y[train_mask]
        X_val = X[~train_mask] # use remaining data for validation
        y_val = y[~train_mask]
...
if __name__ == "__main__":
    ...
    if data:
        ...
        # Prepare datasets
        ml_data = fe.prepare_ml_datasets(features)
